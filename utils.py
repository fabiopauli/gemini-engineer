...
def run_bash_command(command: str) -> Tuple[str, str, int]:
    ...
    if current_platform == "Windows":
        ...
        if not bash_executable:
            return "", "Bash not available on Windows. Install WSL or Git Bash.", 1
    else:
    ...
    try:
        ...
        if bash_check.returncode != 0:
            return "", f"Bash not available or not working properly on {current_platform}", bash_check.returncode
        ...
        return "", f"Bash not found on {current_platform}: {e}", 1
    ...
    try:
        ...
        return "", "Bash command timed out after 30 seconds", 124
    except Exception as e:
        return "", f"Error executing bash command: {e}", 1

def run_powershell_command(command: str) -> Tuple[str, str, int]:
    """
    ...
    if current_platform not in ["Windows", "Linux", "Darwin"]:
        return "", f"PowerShell is not supported on {current_platform}", 1
    ...
    try:
        ...
        if os_check.returncode != 0:
            return "", f"PowerShell not available or not working properly on {current_platform}", os_check.returncode
        ...
        return "", f"PowerShell not found on {current_platform}: {e}", 1
    ...
    try:
        ...
        return "", "PowerShell command timed out after 30 seconds", 124
    except Exception as e:
        return "", f"Error executing PowerShell command: {e}", 1
...
def initialize_logging() -> None:
    """Initialize logging system by creating log directory and session ID."""
    if not logging_context['enabled']:
        return
    ...
    # Generate session ID
    logging_context['current_session_id'] = str(uuid.uuid4())[:8]
    ...
    log_entry = {
        ...
        "type": "session_start",
        "session_id": logging_context['current_session_id'],
        "system_info": {
            "os": os_info['system'],
    ...
    console.print(f"[dim]📝 Logging started: {logging_context['current_log_file'].name}[/dim]")

def log_user_message(content: str, model: str) -> None:
    """Log a user message."""
    if not logging_context['enabled']:
        return
...
def log_api_response(response, user_message: str, model: str, tool_calls: List[Dict[str, Any]] = None) -> None:
    """Log an API response with token usage information."""
    if not logging_context['enabled']:
        return
    ...
    if logging_context['include_token_usage'] and tokens:
        log_entry["output"]["tokens"] = tokens
    
    # Add tool calls if enabled and present
    if logging_context['include_tool_calls'] and tool_calls:
        log_entry["output"]["tool_calls"] = tool_calls
    
    _write_log_entry(log_entry)

def log_tool_execution(tool_call: Dict[str, Any], result: str) -> None:
    """Log tool execution."""
    if not logging_context['enabled'] or not logging_context['include_tool_calls']:
        return
...
def toggle_logging() -> str:
    """Toggle logging on/off and return status message."""
    logging_context['enabled'] = not logging_context['enabled']
    
    if logging_context['enabled']:
        initialize_logging()
        return "✓ Logging enabled. Messages will be saved to logs directory."
    else:
        # Log session end before disabling
        if logging_context['current_session_id']:
            from datetime import datetime
            log_entry = {
                "timestamp": datetime.now().isoformat(),
                "type": "session_end",
        ...
        logging_context['current_session_id'] = None
        logging_context['current_log_file'] = None
        return "✗ Logging disabled. Messages will not be saved."

def get_logging_status() -> str:
    """Get current logging status."""
    if logging_context['enabled']:
        log_file = logging_context.get('current_log_file')
        if log_file:
            return f"📝 Logging enabled → {log_file.name}"
        else:
            return "📝 Logging enabled (initializing...)"
    else:
        return "📝 Logging disabled"

def _write_log_entry(entry: Dict[str, Any]) -> None:
    """Write a log entry to the current log file."""
    if not logging_context['current_log_file']:
        return
    
    try:
        # Check file size and rotate if needed
        if logging_context['current_log_file'].exists():
            file_size_mb = logging_context['current_log_file'].stat().st_size / (1024 * 1024)
            if file_size_mb > logging_context['max_log_file_size_mb']:
                _rotate_log_file()
        
        # Write entry to file
        with open(logging_context['current_log_file'], 'a', encoding='utf-8') as f:
            json.dump(entry, f, ensure_ascii=False)
            f.write('\n')
    
    except Exception as e:
        console.print(f"[yellow]⚠ Logging error: {e}[/yellow]")
...
def _rotate_log_file() -> None:
    ...
    console.print(f"[dim]📝 Log rotated: {logging_context['current_log_file'].name}[/dim]")
...
def _cleanup_old_logs() -> None:
    ...
    try:
        ...
        for file in log_dir.glob("*.json"):
            if file.name.startswith("gemini_log_"):
                log_files.append(file)
        
        log_files.sort(key=lambda f: f.stat().st_mtime)
        
        # Remove excess files
        max_files = logging_context['max_log_files']
        if len(log_files) >= max_files:
            files_to_remove = log_files[:len(log_files) - max_files + 1]
            for file in files_to_remove:
                try:
                    file.unlink()
                    console.print(f"[dim]🗑 Removed old log: {file.name}[/dim]")
                except OSError:
                    pass
    
    except Exception as e:
        console.print(f"[yellow]⚠ Log cleanup error: {e}[/yellow]")