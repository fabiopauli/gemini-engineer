...
MODEL_CONTEXT_LIMITS = {
    ...
    "gemini-2.5-flash": 800000,  # Gemini 2.5 Flash context limit
}
...
REASONER_MODEL = model_config.get("reasoner_model", "gemini-2.5-flash")
...
DEFAULT_LOGGING_CONTEXT = {
    ...
    'log_filename_format': logging_config.get("log_filename_format", "gemini_log_{timestamp}.json"),
    'include_system_messages': logging_config.get("include_system_messages", False),
    'include_tool_calls': logging_config.get("include_tool_calls", True),
    'include_token_usage': logging_config.get("include_token_usage", True),
    'max_log_file_size_mb': logging_config.get("max_log_file_size_mb", 10),
    'max_log_files': logging_config.get("max_log_files", 50),
...
logging_context.update(DEFAULT_LOGGING_CONTEXT)
...
def convert_tools_to_gemini():
    ...
    def convert_schema(schema_def):
        """Recursively convert schema definition."""
        ...
        if schema_type == "string":
            ...
            if "items" in schema_def:
                items_schema = convert_schema(schema_def["items"])
            return types.Schema(
                type="ARRAY",
                description=schema_def.get("description", ""),
                items=items_schema
            ...
            obj_properties = {}
            obj_required = schema_def.get("required", [])
            
            for prop_name, prop_def in schema_def.get("properties", {}).items():
                obj_properties[prop_name] = convert_schema(prop_def)
    ...
    for tool in tools:
        ...
        for prop_name, prop_def in parameters.get("properties", {}).items():
            properties[prop_name] = convert_schema(prop_def)
